...
export function FinalStandings({ standings, myId }: { standings: Standing[]; myId: string }) {
  ...
  return (
    <div className="grid min-h-[100dvh] place-items-center px-5 py-12">
      ...
      >
        <p className="font-display text-sm uppercase tracking-[0.2em] text-mute">{t('final.title')}</p>
        ...
        <p className="mt-1 text-[15px] text-mute">{t('final.winner')}</p>

        <div className="mt-8 overflow-hidden rounded-2xl border border-line bg-surface text-left">
          <div className="flex items-center justify-between border-b border-line px-5 py-2.5">
            <span className="font-display text-xs font-medium text-mute">{t('final.colPlayer')}</span>
            <span className="font-display text-xs font-medium text-mute">{t('final.colPoints')}</span>
          </div>
          {standings.map((s, i) => (
            <div
              key={s.playerId}
            ...
            >
              <span className="flex items-center gap-3">
                ...
                <span className="font-display text-[15px] font-semibold">
                  {s.nickname}
                  {s.playerId === myId && <span className="text-mute"> · {t('final.you')}</span>}
                </span>
              </span>
