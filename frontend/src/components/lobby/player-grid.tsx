    ...
    >
      ...
      <div className="min-w-0">
        <p className="truncate font-display text-[15px] font-semibold">
          {p.name}
          {p.you && <span className="text-mute"> · tú</span>}
        </p>
        {p.host ? (
          ...
          <span className="mt-0.5 block text-xs text-mute">Listo</span>
        )}
      </div>
    </motion.div>
  );
...
function EmptySlot() {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-dashed border-line/70 p-4">
      ...
      <span className="text-sm text-mute">Esperando…</span>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="font-display text-lg font-semibold tracking-tight">Jugadores</h2>
